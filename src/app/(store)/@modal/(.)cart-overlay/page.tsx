  ...
  return (
    <CartAsideContainer withAnimations={true}>
      <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
        ...
        <div className="mt-8">
          <ul role="list" className="-my-6 divide-y divide-neutral-200">
            {cart.lines.map((line) => (
              <li key={line.product.id} className="grid grid-cols-[4rem,1fr,max-content] grid-rows-[auto,auto] gap-x-4 gap-y-2 py-6">
                {line.product.images[0] ? (
                  <div className="col-span-1 row-span-2 bg-neutral-100">
                    <Image className="aspect-square rounded-md object-cover" src={line.product.images[0]} width={80} height={80} alt="" />
                  </div>
                ) : (
                  <div className="col-span-1 row-span-2" />
                )}

                <h3 className="-mt-1 font-semibold leading-tight">{formatProductName(line.product.name, line.product.metadata.variant)}</h3>
                ...
                <p className="self-end text-sm font-medium text-muted-foreground">{t('quantity', { quantity: line.quantity })}</p>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="border-t border-neutral-200 px-4 py-6 sm:px-6">
        <div id="cart-overlay-description" className="flex justify-between text-base font-medium text-neutral-900">
          <p>{t('total')}</p>
          <p>
            {formatMoney({
              amount: total,
