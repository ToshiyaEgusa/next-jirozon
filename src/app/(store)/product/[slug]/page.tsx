...
import { Histories } from '@/ui/Histories'
import { ProductOptionsAndAdd } from '@/ui/ProductOptionsAndAdd'
import { ProductInformation } from '@/ui/ProductInformation'
import { TenpoRecommendation } from '@/ui/TenpoRecommendation'
...
export default async function SingleProductPage({ params, searchParams }: { params: { slug: string }; searchParams: { variant?: string } }) {
  const variants = await Commerce.productGet({ slug: params.slug })
  const selectedVariant = searchParams.variant || variants[0]?.metadata.variant
  const product = variants.find((variant) => variant.metadata.variant === selectedVariant)
    ...
    <article className="pb-12">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-12">
        <div className="lg:col-span-5 lg:col-start-8">
          ...
          <Histories productSlug={product.metadata.slug} />
        ...
        <div className="grid gap-8 lg:col-span-5">
          ...
          {variants.length > 1 && (
            <div className="grid gap-2">
              ...
              <p className="text-sm text-neutral-500">
                申し訳ございません。近年の原材料の高騰や人件費の上昇により、通販の場合はニンニクマシかカラメのみの提供とさせていただきます。何卒ご理解の程、よろしくお願い申し上げます。
              </p>
          ...
          <ProductOptionsAndAdd productId={product.id} productSlug={product.metadata.slug} />
        </div>
      </div>
      <ProductInformation productSlug={product.metadata.slug} />
      <TenpoRecommendation productSlug={product.metadata.slug} />
      <UserReviews productSlug={product.metadata.slug} />
