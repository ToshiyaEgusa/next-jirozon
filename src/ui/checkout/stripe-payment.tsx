...
const fetchSlug = async (productId: string) => {
  if (!process.env.STRIPE_SECRET_KEY) {
    throw new Error('Stripe secret key is not defined')
  }
  const stripe = new Stripe(process.env.STRIPE_SECRET_KEY)
  const product = await stripe.products.retrieve(productId)
  return product.metadata.slug
}

const createHistory = async (productSlug: string, volume: number) => {
  const res = await fetch(`/api/historyDetails?productSlug=${productSlug}&volume=${volume}`, {
    method: 'POST',
  })
  return res.json()
}

const PaymentForm = ({ shippingRates, cartShippingRateId }: { shippingRates: Commerce.MappedShippingRate[]; cartShippingRateId: string | null }) => {
  ...
  const [fieldErrors, setFieldErrors] = useState<Partial<Record<keyof AddressSchema, string[] | null | undefined>>>({})
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = async (event) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        const charge = charges.data[0]?.metadata
        if (!charge) return
        const prodMetadata = Object.entries(charge)
          .filter(([key, value]) => key.startsWith('prod_'))
          .reduce((obj: Record<string, string>, [key, value]) => {
            obj[key] = value
            return obj
          }, {})

        const slugObject: Record<string, string> = {}
        for (const [key, value] of Object.entries(prodMetadata)) {
          try {
            const slug = await fetchSlug(key)
            if (slug) {
              slugObject[slug] = value
            }
          } catch (error) {
            console.error(error)
          }
        ...
        for (const [key, value] of Object.entries(slugObject)) {
          try {
            await createHistory(key, parseInt(value))
          } catch (error) {
            console.error(error)
          }
        }
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4">
      ...
      {readyToRender && (
        <Label className="flex flex-row items-center gap-x-2" aria-controls="billingAddressCollapsibleContent" aria-expanded={!sameAsShipping}>
          <Checkbox
      ...
      {readyToRender && (
        <Collapsible className="" open={!sameAsShipping}>
          <CollapsibleContent id="billingAddressCollapsibleContent" className="CollapsibleContent">
            <fieldset aria-hidden={sameAsShipping} tabIndex={sameAsShipping ? -1 : undefined} className={`grid gap-6 rounded-lg border p-4`}>
              <legend className="-ml-1 whitespace-nowrap px-1 text-sm font-medium">{t('billingAddressTitle')}</legend>
              <BillingAddressSection values={billingAddressValues} onChange={setBillingAddressValues} errors={fieldErrors} />
            </fieldset>
          </CollapsibleContent>
        </Collapsible>
      )}
      ...
      {readyToRender && (
        ...
        >
          {isBillingAddressPending || isLoading || isTransitioning ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : t('payNowButton')}
        </Button>
      )}
    </form>
  )
